...
>   - OCR training data in `services/document-document-validation-service/tests/ocr-training/` (see tree above)
> - **Testing and Lint:**
...
- **Static Assets**: SVG icons from `frontend/assets/icons` via a Tailwind plugin; OCR training images loaded at test‑runtime from `services/document-document-validation-service/tests/ocr-training/**`.
...
..
    ...
    "build:wasm:validation": "wasm-pack build services/document-document-validation-service --target web --out-dir pkg/validation",
    "build:wasm:country": "wasm-pack build services/location-service --target web --out-dir pkg/country",
...
| ----------------------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| **Document Processing**       | DNI/NIE checksum, MRZ parsing, smart‑rotation detection (projection + Hough), confidence scoring | `document-document-validation-service` + frontend WASM helpers |
...
| **Advanced Validation**       | Phone (+ country code), email regex + MX check, address autocomplete via Google Places           | `document-document-validation-service` endpoints               |
...
- Maintained proper separation: services/document-document-validation-service/, services/booking-service/
...
- Services properly organized: services/document-document-validation-service/, services/booking-service/
