...
When we try to run SeaORM migrations against Turso using the `libsql` crate **in the same binary** as `sea-orm-migration`/SQLx (SQLite), the build fails at link-time with “multiple definition of sqlite3\_\*”.
