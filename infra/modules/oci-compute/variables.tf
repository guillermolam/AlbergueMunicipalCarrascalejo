...
variable "region" {
  type        = string
  description = "The OCI region to deploy resources. Must be the tenancy home region for Always Free A1 instances."
  default     = "eu-madrid-1"
}
...
variable "subnet_cidr_block" {
  type        = string
  description = "The CIDR block for the subnet (must be within vcn_cidr_block)"
  default     = "10.1.0.0/24"
...
variable "instance_shape" {
  type        = string
  description = "The OCI compute shape"
...
variable "docker_volume_size_gb" {
  type        = number
  description = "The size of the secondary block volume in GBs (mounted at /mnt/data)"
  default     = 150
...
variable "kms_key_id" {
  type        = string
  description = "The OCID of the KMS key for volume encryption. Null = Oracle-managed keys."
  default     = null
}
...
variable "enable_unrestricted_egress" {
  type        = bool
  description = "Allow all outbound traffic. When false, only egress_security_rules are applied."
  default     = true
}
...
variable "timezone" {
  type        = string
  description = "IANA timezone for the instance (e.g. Europe/Madrid)"
  default     = "Europe/Madrid"
}

variable "install_runtipi" {
  type        = bool
  description = "Whether to install RunTipi homeserver"
  default     = false
}

variable "runtipi_main_network_subnet" {
  type        = string
  description = "Docker network subnet for RunTipi containers"
  default     = "172.18.0.0/16"
}

variable "runtipi_reverse_proxy_ip" {
  type        = string
  description = "Static IP for RunTipi Traefik reverse proxy"
  default     = "172.18.0.254"
}

variable "runtipi_adguard_ip" {
  type        = string
  description = "Static IP for AdGuard"
  default     = "172.18.0.253"
}

variable "wireguard_client_configuration" {
  type        = string
  description = "WireGuard client configuration (wg0.conf content). Empty = no WireGuard."
  default     = ""
  sensitive   = true
}
...
variable "instance_image_ocids_by_region" {
  ...
  default = {
    # Canonical Ubuntu 24.04 Minimal for ARM64 — see https://docs.oracle.com/en-us/iaas/images/
