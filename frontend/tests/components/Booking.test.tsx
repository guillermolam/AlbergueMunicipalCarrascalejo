...
describe("Booking", () => {
  ...
  it("displays warning when no room is selected", () => {
    ...
    expect(
      screen.getByText(
        "Por favor selecciona una habitación arriba para continuar",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("validates required fields", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("La fecha de entrada es requerida"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("La fecha de salida es requerida"),
      ).toBeInTheDocument();
      expect(
      ...
      expect(
        screen.getByText("El teléfono debe tener al menos 9 dígitos"),
      ).toBeInTheDocument();
    });
  });
