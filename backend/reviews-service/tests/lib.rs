...
async fn test_google_reviews_endpoint() {
    ...
    assert_eq!(json_body.get("average_rating").unwrap().as_f64().unwrap(), 4.666666666666667);
    let source_breakdown = json_body.get("source_breakdown").unwrap().as_object().unwrap();
...
async fn test_booking_reviews_endpoint() {
    ...
    let source_breakdown = json_body.get("source_breakdown").unwrap().as_object().unwrap();
    assert_eq!(source_breakdown.len(), 1);
    assert_eq!(source_breakdown.get("Booking.com").unwrap().as_u64().unwrap(), 3);
}
...
async fn test_invalid_endpoint() {
    ...
    assert_eq!(json_body.get("error").unwrap().as_str().unwrap(), "Not Found");
    assert_eq!(json_body.get("message").unwrap().as_str().unwrap(), "Reviews endpoint not found");
}
