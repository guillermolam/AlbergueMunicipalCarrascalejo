...
import { dailyGoalKm, currentStageProgress, remainingDays, setDailyGoal, setStageProgress, syncProgressToServer } from '../stores/app'
...
function render() {
  document.querySelectorAll("[data-store='dailyGoalKm']").forEach((el) => setText(el, String(dailyGoalKm.get())))
  document.querySelectorAll("[data-store='stageProgress']").forEach((el) => setText(el, String(currentStageProgress.get())))
  document.querySelectorAll("[data-store='remainingDays']").forEach((el) => setText(el, String(remainingDays.get())))
}
