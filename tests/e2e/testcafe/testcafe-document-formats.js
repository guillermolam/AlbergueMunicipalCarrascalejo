...
%%EOF`;
  ...
  try {
    ...
    await t
      .expect(fileInput.exists)
      .ok("File input should exist for PDF upload");

    // Check if PDF processing is handled
    const processingIndicator = Selector(
      '[data-testid*="processing"], .processing, .spinner',
    );
    ...
    if (extractedValue === "") {
      ...
      await t
        ...
        .ok("Manual entry should be available for PDF")
    ...
    console.log(
      "⚠️ PDF upload not fully supported - providing manual entry fallback",
    );
    ...
    await t
      ...
      .typeText(
        Selector('input[name="lastName1"], input[id*="lastName"]'),
        "DOE",
      );
  }
});
...
test("Process DOCX document with passport pictures", async (t) => {
  ...
  try {
    ...
    const supportMessage = Selector(
      '[data-testid*="file-type"], .file-type-info, .supported-formats',
    );
...
test("Validate supported document file formats", async (t) => {
  ...
  for (const supportedFile of supportedFormats) {
    try {
      await t.setFilesToUpload(fileInput, [supportedFile]).wait(2000);

      // Should not show format error for supported files
      const formatError = Selector('.error, [data-testid*="error"]').withText(
        /format|tipo|archivo/i,
      );
      await t
        .expect(formatError.exists)
        .notOk(`Supported format should not show error: ${supportedFile}`);
...
test("Validate document file size limits", async (t) => {
  ...
  for (const testFile of testFiles) {
    try {
      await t.setFilesToUpload(fileInput, [testFile]).wait(2000);

      // Check if size validation message appears
      const sizeWarning = Selector(
        '[data-testid*="size"], .file-size, .size-warning',
      );
...
test("Test drag and drop document upload", async (t) => {
  ...
  if (await dropZone.exists) {
    ...
    await t
      .hover(dropZone)
      .expect(dropZone.exists)
      .ok("Drop zone should be available");
    ...
    await t
      .expect(fileInput.exists)
      .ok("File input should be available as fallback");
...
test("Handle file upload errors gracefully", async (t) => {
  ...
  try {
    ...
    const errorMessage = Selector(
      '.error, [data-testid*="error"], .upload-error',
    );
    if (await errorMessage.exists) {
      ...
      await t
        .expect(manualEntry.exists)
        .ok("Manual entry should be available on upload error");
    }
    ...
    console.log(
      "ℹ️ File upload error scenario tested - manual entry available",
    );
...
test("Test file upload accessibility features", async (t) => {
  ...
  await t
    .expect(fileInput.hasAttribute("accept"))
    .ok("File input should have accept attribute")
    .expect(fileInput.exists)
    .ok("File input should be keyboard accessible");

  // Check for labels and instructions
  const uploadLabel = Selector("label").withAttribute(
    "for",
    fileInput.getAttribute("id"),
  );
  const uploadInstructions = Selector(
    '[data-testid*="instructions"], .upload-instructions, .help-text',
  );
  ...
  await t
    ...
    .expect(fileInput.focused)
    .ok("File input should be focusable with keyboard");
