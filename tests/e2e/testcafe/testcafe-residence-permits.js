...
test("Process TIE residence permit card", async (t) => {
  ...
  await t
    ...
    .click(
      Selector('option, [role="option"]').withText(/TIE|Residence|Permiso/i),
    );
  ...
  const documentNumberField = Selector(
    'input[name="documentNumber"], input[id*="document"]',
  );
  await t
    .expect(documentNumberField.value)
    .match(/^[XYZ]\d{7}[A-Z]$/i, "TIE should extract associated NIE number");

  // Verify personal data extraction
  await t
    .expect(Selector('input[name="firstName"], input[id*="firstName"]').value)
    .notEql("", "First name should be extracted from TIE")
    .expect(Selector('input[name="lastName1"], input[id*="lastName"]').value)
    .notEql("", "Last name should be extracted from TIE");
...
test("Detect work authorization from TIE card", async (t) => {
  ...
  const workAuthField = Selector(
    'input[name="workAuthorization"], select[name="workPermit"], [data-testid*="work"]',
  );
  if (await workAuthField.exists) {
    const workAuthValue = await workAuthField.value;
    await t
      .expect(workAuthValue)
      .match(
        /yes|authorized|permitido|sí/i,
        "Work authorization should be detected",
      );
  }

  // Verify residence type is detected
  const residenceTypeField = Selector(
    'select[name="residenceType"], input[name="residenceType"]',
  );
  if (await residenceTypeField.exists) {
    await t
      .expect(residenceTypeField.value)
      .match(
        /work|trabajo|worker|fronterizo/i,
        "Work residence type should be detected",
      );
  }
...
test("Process student residence permit", async (t) => {
  ...
  await t
    ...
    .click(
      Selector('option, [role="option"]').withText(
        /TIE|Residence|Student|Estudiante/i,
      ),
    );
  ...
  await t
    .expect(documentField.value)
    .notEql("", "Document number should be extracted from student permit");

  // Check if student status is detected
  const statusField = Selector(
    'select[name="status"], input[name="studentStatus"], [data-testid*="student"]',
  );
  if (await statusField.exists) {
    ...
    await t
      .expect(statusValue)
      .match(
        /student|estudiante|study|estudios/i,
        "Student status should be detected",
      );
  }
...
test("Process border worker residence permit", async (t) => {
  ...
  await t
    ...
    .click(
      Selector('option, [role="option"]').withText(/TIE|Border|Fronterizo/i),
    );
  ...
  await t
    .expect(documentField.value)
    .notEql("", "Border worker permit number should be extracted");

  // Check for special border worker status
  const workerTypeField = Selector(
    'select[name="workerType"], input[name="borderWorker"]',
  );
  if (await workerTypeField.exists) {
    await t
      .expect(workerTypeField.value)
      .match(
        /border|fronterizo|cross.border/i,
        "Border worker type should be detected",
      );
  }
...
test("Extract both TIE permit number and associated NIE", async (t) => {
  ...
  await t
    .expect(documentField.value)
    .match(/^[XYZ]\d{7}[A-Z]$/i, "NIE number should be extracted as primary");

  // Check if TIE permit number is stored separately
  const tieNumberField = Selector(
    'input[name="tieNumber"], input[name="permitNumber"], [data-testid*="tie"]',
  );
  if (await tieNumberField.exists) {
    const tieValue = await tieNumberField.value;
    await t
      .expect(tieValue)
      .notEql("", "TIE permit number should also be extracted");
  }
...
test("Extract residence permit expiry date", async (t) => {
  ...
  const expiryField = Selector(
    'input[name="expiryDate"], input[name="validUntil"], [data-testid*="expiry"]',
  );
  if (await expiryField.exists) {
    ...
    await t
      ...
      .match(
        ...
        "Expiry date should be in valid format",
      )
      .expect(expiryValue)
      .notEql("", "Expiry date should be extracted");
  }

  // Verify document is not expired
  const warningMessage = Selector(
    '.warning, [data-testid*="warning"], .text-yellow-500',
  );
  const errorMessage = Selector(
    '.error, [data-testid*="error"], .text-red-500',
  );

  // Should not show expired document error for future dates
  await t
    .expect(errorMessage.withText(/expired|caducado/i).exists)
    .notOk("Should not show expired error for valid documents");
...
test("Recognize different residence permit categories", async (t) => {
  ...
  for (const permitTest of permitTests) {
    ...
    const permitTypeField = Selector(
      'select[name="permitType"], input[name="residenceType"]',
    );
    if (await permitTypeField.exists) {
      ...
      await t
        .expect(permitValue)
        .match(
          permitTest.expectedType,
          `Permit type should be recognized for ${permitTest.image}`,
        );
    }
...
test("Handle low-quality residence permit images", async (t) => {
  ...
  if (extractedValue === "" || extractedValue.length < 5) {
    ...
    await t
      .expect(Selector('[data-testid*="manual"], .manual-entry').exists)
      .ok("Manual entry should be available for poor quality images")
    ...
    await t
      .expect(extractedValue.length)
      .gte(5, "Should extract some recognizable data");
  }
...
test("Validate residence permit data integrity", async (t) => {
  ...
  await t
    .expect(Selector('.error, [data-testid*="error"], .text-red-500').exists)
    .ok("Invalid format should show error");
  ...
  await t
    .expect(Selector('.error, [data-testid*="error"], .text-red-500').exists)
    .notOk("Valid format should not show error");
