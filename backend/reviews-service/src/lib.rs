...
// ── Public types (always compiled — used by integration tests) ────────────────
...
// ── Pure helper functions (always compiled) ───────────────────────────────────

pub fn calculate_average_rating(reviews: &[Review]) -> f32 {
    if reviews.is_empty() {
        return 0.0;
    }
...
pub fn create_source_breakdown(reviews: &[Review]) -> HashMap<String, u32> {
...
// ── WASM handler (only compiled for wasm32 target) ────────────────────────────
// spin_sdk::http_component generates a wasi:http/incoming-handler component
// export that the native linker cannot resolve.  Gate everything that touches
// Spin HTTP or the component macro behind cfg(target_arch = "wasm32").

#[cfg(target_arch = "wasm32")]
mod wasm_handler {
    use super::{
        calculate_average_rating, create_source_breakdown, ErrorResponse, Review, ReviewsResponse,
    };
    use anyhow::Result;
    ...
    async fn handle_review_stats() -> Result<Response> {
        let stats = serde_json::json!({
            "total_reviews": 6,
            "average_rating": 4.7,
            "rating_distribution": { "5": 4, "4": 2, "3": 0, "2": 0, "1": 0 },
            "sources": { "Google": 3, "Booking.com": 3 },
