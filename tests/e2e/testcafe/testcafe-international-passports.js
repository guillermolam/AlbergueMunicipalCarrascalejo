...
test("Process US Passport with MRZ", async (t) => {
  ...
  await t
    ...
    .match(/^[A-Z0-9]{6,9}$/, "US Passport number should be extracted")
    .expect(documentField.value)
    .notEql("", "Passport number should not be empty");

  // Verify personal data from MRZ
  await t
    .expect(Selector('input[name="firstName"], input[id*="firstName"]').value)
    .notEql("", "First name should be extracted from passport")
    .expect(Selector('input[name="lastName1"], input[id*="lastName"]').value)
    .notEql("", "Last name should be extracted from passport");
  ...
  if (await nationalityField.exists) {
    ...
    await t
      .expect(nationalityValue)
      .match(
        /USA|United States|American/i,
        "US nationality should be detected",
      );
  }
...
test("Process Chinese Passport", async (t) => {
  ...
  for (const passportImage of chinesePassports) {
    await t
      ...
      .click(
        Selector('option, [role="option"]').withText(/Passport|Pasaporte/i),
      );
    ...
    if (extractedValue !== "") {
      await t
        .expect(extractedValue)
        .match(/^[A-Z0-9]{8,9}$/, "Chinese passport number should be extracted")
        .expect(
          Selector('input[name="firstName"], input[id*="firstName"]').value,
        )
        .notEql("", "Name should be extracted");
      ...
      if (await nationalityField.exists) {
        ...
        await t
          ...
          .match(
            /China|Chinese|CHN/i,
            "Chinese nationality should be detected",
          );
      }
    } else {
  ...
  for (const passportImage of russianPassports) {
    ...
    if (extractedValue !== "" && extractedValue.length >= 6) {
      await t
        ...
        .match(
          /^[0-9]{9}$|^[A-Z0-9]{8,10}$/,
          "Russian passport number should be valid format",
        )
        .expect(
          Selector('input[name="firstName"], input[id*="firstName"]').value,
        )
        .notEql("", "Name should be extracted from Russian passport");
      ...
      if (await nationalityField.exists) {
        ...
        await t
          .expect(nationalityValue)
          .match(
            /Russia|Russian|RUS|РФ/i,
            "Russian nationality should be detected",
          );
      }
    } else {
...
test("Process Barbados Passport", async (t) => {
  ...
  if (extractedValue !== "") {
    await t
      .expect(extractedValue)
      .match(/^[A-Z0-9]{6,9}$/, "Barbados passport number should be valid")
      .expect(Selector('input[name="firstName"], input[id*="firstName"]').value)
      .notEql("", "Name should be extracted");
    ...
    if (await nationalityField.exists) {
      ...
      await t
        .expect(nationalityValue)
        .match(
          /Barbados|Barbadian|BRB/i,
          "Barbados nationality should be detected",
        );
    }
  } else {
...
test("Parse MRZ from international passports", async (t) => {
  ...
  await t
    .expect(documentField.value)
    .notEql("", "Passport number should be extracted from MRZ")
    .expect(firstNameField.value)
    .notEql("", "First name should be extracted from MRZ")
    .expect(lastNameField.value)
    .notEql("", "Last name should be extracted from MRZ");
  ...
  if (await birthDateField.exists) {
    ...
    if (birthDateValue !== "") {
      await t
        .expect(birthDateValue)
        .match(
          /\d{2}[\/\-]\d{2}[\/\-]\d{4}|\d{4}[\/\-]\d{2}[\/\-]\d{2}/,
          "Birth date should be in valid format",
        );
    }
  }
...
test("Extract passport expiry dates", async (t) => {
  ...
  const expiryField = Selector(
    'input[name="expiryDate"], input[name="passportExpiry"], [data-testid*="expiry"]',
  );
  ...
  if (await warningMessage.exists) {
    console.log(
      "⚠️ Expired passport warning shown (expected for 2004 passport)",
    );
  }
  ...
  if (await genderField.exists) {
    ...
    if (genderValue !== "") {
      await t
        .expect(genderValue)
        .match(
          /^[MF]$|Male|Female|Masculino|Femenino/i,
          "Gender should be detected from passport",
        );
    }
  }
...
test("Extract passport photo for verification", async (t) => {
  ...
  const photoPreview = Selector(
    '[data-testid*="photo"], .passport-photo, img[alt*="passport"]',
  );
  if (await photoPreview.exists) {
    await t
      .expect(photoPreview.exists)
      .ok("Passport photo should be extracted and displayed");
  }
  ...
  for (const passportTest of passportFormats) {
    ...
    if (extractedValue !== "" && extractedValue.length >= 6) {
      await t
        .expect(extractedValue)
        .match(
          passportTest.expectedPattern,
          `${passportTest.country} passport format should be recognized`,
        );
    } else {
...
test("Validate international passport data", async (t) => {
  ...
  for (const invalidFormat of invalidFormats) {
    ...
    const errorExists = await Selector(
      '.error, [data-testid*="error"], .text-red-500',
    ).exists;
    if (invalidFormat !== "" && errorExists) {
      await t
        .expect(errorExists)
        .ok(`Invalid format "${invalidFormat}" should show error`);
    }
  }
  ...
  await t
    ...
    .notOk("Valid passport number should not show error");
