...
mod edge_case_tests {
    ...
    async fn test_rate_limit_with_zero_max_requests() {
        ...
        assert!(result.is_ok(), "Zero max requests should be handled gracefully");
        let response = result.unwrap();
    ...
    async fn test_rate_limit_with_very_large_max_requests() {
        ...
        assert!(result.is_ok(), "Large max requests should be handled gracefully");
        let response = result.unwrap();
        assert!(response.allowed, "First request should be allowed");
    ...
    async fn test_extract_client_id_with_malformed_headers() {
        // Test with non-UTF8 headers
        let req = Request::builder()
            .uri("http://example.com/test")
            .method("GET")
            .header("x-forwarded-for", "\xFF\xFF\xFF")
            .body(vec![])
            .unwrap();

        let client_id = extract_client_id(&req);
        assert_eq!(client_id, "unknown", "Malformed headers should default to unknown");
    }
    ...
    async fn test_rate_limit_check_with_unicode_endpoint() {
        ...
        assert!(result.is_ok(), "Unicode endpoints should be handled gracefully");
        let response = result.unwrap();
        assert!(response.allowed, "First request should be allowed");
    }
    ...
    async fn test_rate_limit_check_with_overflow_scenarios() {
        ...
        assert!(result.is_ok(), "Overflow scenarios should be handled gracefully");
        let response = result.unwrap();
    ...
    async fn test_rate_limit_check_with_boundary_timestamps() {
        ...
        let result = calculate_rate_limit(Some(existing), current_time, 60, 10);
