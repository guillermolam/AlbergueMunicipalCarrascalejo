...
export class PerformanceMonitor {
  ...
  checkPerformanceTargets(): { violations: string[]; warnings: string[] } {
    ...
    if (metrics.lcp && metrics.lcp > performanceTargets.lcp) {
      violations.push(
        `LCP (${metrics.lcp.toFixed(0)}ms) exceeds target (${performanceTargets.lcp}ms)`
      );
    } else if (metrics.lcp && metrics.lcp > performanceTargets.lcp * 0.8) {
      warnings.push(
        `LCP (${metrics.lcp.toFixed(0)}ms) approaching target (${performanceTargets.lcp}ms)`
      );
    }
    ...
    if (metrics.fcp && metrics.fcp > performanceTargets.fcp) {
      violations.push(
        `FCP (${metrics.fcp.toFixed(0)}ms) exceeds target (${performanceTargets.fcp}ms)`
      );
    } else if (metrics.fcp && metrics.fcp > performanceTargets.fcp * 0.8) {
      warnings.push(
        `FCP (${metrics.fcp.toFixed(0)}ms) approaching target (${performanceTargets.fcp}ms)`
      );
    }
    ...
    if (metrics.fid && metrics.fid > performanceTargets.fid) {
      violations.push(
        `FID (${metrics.fid.toFixed(0)}ms) exceeds target (${performanceTargets.fid}ms)`
      );
    } else if (metrics.fid && metrics.fid > performanceTargets.fid * 0.8) {
      warnings.push(
        `FID (${metrics.fid.toFixed(0)}ms) approaching target (${performanceTargets.fid}ms)`
      );
    }
    ...
    if (metrics.cls && metrics.cls > performanceTargets.cls) {
      ...
      warnings.push(
        `CLS (${metrics.cls.toFixed(3)}) approaching target (${performanceTargets.cls})`
      );
    }
    ...
    if (metrics.ttfb && metrics.ttfb > performanceTargets.ttfb) {
      violations.push(
        `TTFB (${metrics.ttfb.toFixed(0)}ms) exceeds target (${performanceTargets.ttfb}ms)`
      );
    } else if (metrics.ttfb && metrics.ttfb > performanceTargets.ttfb * 0.8) {
      warnings.push(
        `TTFB (${metrics.ttfb.toFixed(0)}ms) approaching target (${performanceTargets.ttfb}ms)`
      );
    }
    ...
    if (metrics.bundleSize && metrics.bundleSize > performanceTargets.bundleSize) {
      violations.push(
        `Bundle size (${(metrics.bundleSize / 1024).toFixed(2)}KB) exceeds target (${(performanceTargets.bundleSize / 1024).toFixed(2)}KB)`
      );
    } else if (metrics.bundleSize && metrics.bundleSize > performanceTargets.bundleSize * 0.8) {
      warnings.push(
        `Bundle size (${(metrics.bundleSize / 1024).toFixed(2)}KB) approaching target (${(performanceTargets.bundleSize / 1024).toFixed(2)}KB)`
      );
    }
