...
fixture`NIE Document Processing Tests`.page`http://localhost:5000`.beforeEach(
  async (t) => {
...
test("Process NIE document with Y-format number", async (t) => {
  ...
  await t
    .expect(documentNumberField.value)
    .match(/^Y\d{7}[A-Z]$/i, "NIE Y-format should be extracted correctly");
...
test("Process NIE document with Z-format number", async (t) => {
  ...
  if (documentValue.match(/^Z\d{7}[A-Z]$/i)) {
    await t
      .expect(documentNumberField.value)
      .match(/^Z\d{7}[A-Z]$/i, "NIE Z-format should be extracted correctly");
  } else {
...
test("Validate NIE checksum calculation", async (t) => {
  ...
  await t
    ...
    .notOk("Valid NIE should not show error");
  ...
  await t
    .expect(Selector('.error, [data-testid*="error"], .text-red-500').exists)
    .ok("Invalid NIE checksum should show error");
...
test("Process EU registration certificate", async (t) => {
  ...
  await t
    .expect(Selector('input[name="firstName"], input[id*="firstName"]').value)
    .notEql("", "Name should be extracted from EU certificate")
    .expect(
      Selector('input[name="nationality"], input[id*="nationality"]').value,
    )
    .match(/EU|European|España|Spain/i, "EU nationality should be detected");
...
test("Process permanent residence certificate", async (t) => {
  ...
  await t
    ...
    .click(
      Selector('option, [role="option"]').withText(/Residence|NIE|Other/i),
    );
  ...
  await t
    .expect(extractedNumber)
    .notEql(
      "",
      "Document number should be extracted from permanent residence certificate",
    );

  // Check if residence type is detected
  const residenceTypeField = Selector(
    'select[name="residenceType"], input[name="residenceType"], [data-testid*="residence"]',
  );
  if (await residenceTypeField.exists) {
    await t
      .expect(residenceTypeField.value)
      .match(
        /permanent|permanente/i,
        "Permanent residence type should be detected",
      );
  }
...
test("Handle damaged or unclear NIE documents", async (t) => {
  ...
  if (extractedValue === "") {
    ...
    await t
      .expect(
        Selector('[data-testid*="manual"], .manual-entry, [class*="manual"]')
          .exists,
      )
      .ok("Manual entry option should be available")
    ...
    await t
      ...
      .gte(1, "Should extract some document information");
  }
...
test("Process NIE document with multiple languages", async (t) => {
  ...
  const languageSelector = Selector(
    '[data-testid="language-selector"], select[name="language"]',
  );
  ...
  await t
    .expect(
      Selector('input[name="documentNumber"], input[id*="document"]').value,
    )
    .match(
      /^[XYZ]\d{7}[A-Z]$/i,
      "NIE should be extracted in Spanish interface",
    );
  ...
  await t
    .expect(Selector('input[name="firstName"], input[id*="firstName"]').exists)
    .ok("Form should work in English interface");
...
test("NIE document processing performance", async (t) => {
  ...
  await t.setFilesToUpload(Selector('input[type="file"]').nth(0), [
    "./attached_assets/nie_1753387245232.jpg",
  ]);
  ...
  await t
    ...
    .notEql("", "Document should be processed", { timeout: 10000 });
  ...
  await t
    .expect(processingTime)
    .lt(8000, "NIE processing should complete within 8 seconds");
