...
test("DNI Upload and OCR Processing", async (t) => {
  ...
  await t.setFilesToUpload(fileUpload, [
  ...
  const processingIndicator = Selector(
    '.ocr-processing, .processing, [data-testid="processing"]',
  );
  if (await processingIndicator.exists) {
    console.log("🔄 OCR processing detected, waiting for completion...");
    await t
      .expect(processingIndicator.exists)
      .notOk("OCR processing completed", { timeout: 60000 });
  }

  // Step 7: Verify form fields are populated
  const formFields = {
    firstName: Selector(
      'input[name="firstName"], input[placeholder*="first"], input[placeholder*="nombre"]',
    ),
    lastName: Selector(
      'input[name="lastName"], input[name="lastName1"], input[placeholder*="last"], input[placeholder*="apellido"]',
    ),
    documentNumber: Selector(
      'input[name="documentNumber"], input[name="dni"], input[placeholder*="document"], input[placeholder*="DNI"]',
    ),
    birthDate: Selector(
      'input[name="birthDate"], input[name="dateOfBirth"], input[type="date"]:not(:first-child)',
    ),
  };
  ...
  console.log(
    `📊 Extracted ${extractedFields} out of ${Object.keys(formFields).length} fields`,
  );

  // Verify at least some data was extracted
  await t
    .expect(extractedFields)
    .gte(1, "At least one field should be populated by OCR");
  ...
  if (await formFields.documentNumber.exists) {
    ...
    if (dniValue && dniValue.length > 0) {
      ...
      await t
        .expect(dniPattern.test(dniValue.toUpperCase()))
        .ok(`DNI format should be valid: ${dniValue}`);
    }
  }
