...
jest.mock("../../client/src/components/ui/alert", () => ({
  Alert: ({ children, className }: any) => (
    <div className={className} data-testid="mock-alert">
      {children}
    </div>
  ),
  AlertDescription: ({ children, className }: any) => (
    <div className={className} data-testid="mock-alert-description">
      {children}
    </div>
  ),
}));
...
jest.mock("../../client/src/components/ui/progress", () => ({
  Progress: ({ value, className }: any) => (
    <div className={className} data-testid="mock-progress" data-value={value} />
  ),
}));
...
describe("MultiDocumentCapture Component", () => {
  ...
  describe("Shallow Rendering", () => {
    ...
    it("should render upload areas for front and back", () => {
      const wrapper = shallow(<MultiDocumentCapture {...mockProps} />);
      expect(wrapper.find('[data-testid="mock-card"]').length).toBeGreaterThan(
        0,
      );
    });
  ...
  describe("Full Mount Rendering", () => {
    ...
    it("should render upload buttons", () => {
      const wrapper = renderWithProvider();
      expect(
        wrapper.find('[data-testid="mock-button"]').length,
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe("Document Processing", () => {
    ...
    it("should show completion status when document is processed", () => {
      ...
      wrapper.setState({
        frontOCR: {
          success: true,
          extractedData: { documentNumber: "12345678Z" },
        },
